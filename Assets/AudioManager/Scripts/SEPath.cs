namespace KanKikuchi.AudioManager{

/// <summary>
/// SEファイルへのパスを定数で管理するクラス
/// </summary>
public static class SEPath{

	public const string BUBBLE_BIRTH     = "SE/Bubble_Birth";
	public const string BUBBLE_DEATH     = "SE/Bubble_Death";
	public const string JINGLE10         = "SE/Jingle/jingle10";
	public const string SE_BUBBLE_BIG    = "SE/タイトル演出/泡ふくらむ音/SE_BubbleBig";
	public const string SE_BUBBLE_DISON  = "SE/宝箱/SE_BubbleDison";
	public const string SE_BUBBLE_RISE   = "SE/タイトル演出/泡の浮上音/音人/SE_BubbleRise";
	public const string SE_BUBBLE_SEX    = "SE/SE_BubbleSex";
	public const string SE_GOAL_CLOSE    = "SE/宝箱/SE_GoalClose";
	public const string SE_GOAL_KIRAKIRA = "SE/宝箱/SE_GoalKirakira";
	public const string SE_GOAL_OPEN     = "SE/宝箱/SE_GoalOpen";
	public const string SE_NEEDLE        = "SE/SE_Needle";
	public const string SE_STEAGE_SELECT = "SE/タイトル演出/ボタン決定音/効果音ラボ/SE_SteageSelect";
	public const string SYSTEM20         = "SE/system20";
	public const string SYSTEM23         = "SE/system23";

}

}
